...
onMounted(() => {
  ...
  // 如果初始有值，创建成功状态的文件对象
  if (innerValue.value) {
    tempImgUrl.value = resolveUrl(innerValue.value);
    file.value = {
      status: "success",
...
watch(
  ...
  (newVal) => {
    ...
    if (newVal) {
      tempImgUrl.value = resolveUrl(newVal);
      file.value = {
        status: "success",
...
const handleHttpRequest = async (options: UploadRequestOptions) => {
  ...
  try {
    ...
    const simageUrl = resolveUrl(res.path);
    tempImgUrl.value = simageUrl;
    if (res.path) {
      file.value = {
        status: "success",
        url: simageUrl,
