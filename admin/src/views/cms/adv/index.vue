<template>
  <el-container class="h-full">
    <el-aside width="240px" class="h-full">
      ...
      >
        <div
          class="mb-2 flex items-center justify-between border-b pb-2 dark:border-slate-750"
        >
          <span>广告栏目</span>
          ...
          <template #bodyCell="{ text, column, record }">
            ...
            <template v-if="column.key === 'imgUrl'">
              <el-image
                ...
                fit="contain"
              />
              <span v-if="!record.imgUrl">无图</span>
            </template>
...
const fetchColumn = async () => {
  let res = await fetchAdvColumnList();
...
const handleColumnDelete = async () => {
  await deleteAdvColumn(selectColumn.value?.id);
  ElMessage.success("删除成功");
