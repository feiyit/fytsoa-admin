...
export function resolveUrl(
  ...
  // 运行时配置优先（public/runtime-config.js）
  const runtimeConfig =
    (typeof window !== "undefined" && (window as any).__APP_CONFIG__) || {};
  const runtimeBase = runtimeConfig.API_BASE_URL as string | undefined;

  // 从环境变量获取服务器基础地址
  let baseUrl = (runtimeBase || (import.meta.env[envKey] as string) || "").trim();
  baseUrl = baseUrl.replace(/\/api\/?$/i, "");

  // 如果没有配置 baseUrl，则回退到当前站点域名，确保返回绝对地址
  if (!baseUrl) {
    const origin = typeof window !== "undefined" ? window.location.origin : "";
    if (!origin) {
      console.warn(
        `环境变量${envKey}未配置，且无法获取 window.location.origin，将直接返回相对路径: ${trimmedUrl}`,
      );
      return trimmedUrl;
    }
    baseUrl = origin;
...
export function formatRelativeDateDetail(
  ...
  try {
    ...
    const diffDays = Math.floor(
      (todayZero - targetZero) / (1000 * 60 * 60 * 24),
    );
