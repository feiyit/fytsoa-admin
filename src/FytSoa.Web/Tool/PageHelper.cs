...
namespace FytSoa.Web
{
    public class PageHelper
    {
        public static string Htmls(int page, int pagesize, int total, string url)
        {
            ...
            sb.AppendLine($"<li class=\"page-item {prevDisabled}\">");
            sb.AppendLine($"    <a class=\"page-link\" href=\"{prevHref}\" tabindex=\"-1\">上一页</a>");
            sb.AppendLine("</li>");
            ...
            for (int i = 1; i <= totalPages; i++)
            {
                ...
                sb.AppendLine($"<li class=\"page-item {active}\"><a class=\"page-link\" href=\"{pageHref}\">{i}</a></li>");
            }

            // 3. 下一页按钮
            string nextDisabled = page >= totalPages ? "disabled" : "";
            string nextHref = page < totalPages ? $"{url}{page + 1}" : "#";
            sb.AppendLine($"<li class=\"page-item {nextDisabled}\">");
            sb.AppendLine($"    <a class=\"page-link\" href=\"{nextHref}\">下一页</a>");
            sb.AppendLine("</li>");
