...
public class AopActionFilter : IAsyncActionFilter
{
    ...
    public AopActionFilter(ICapPublisher capBus,ICacheService cacheService)
    {
        _capBus = capBus;
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (!SkipLogging(context))
        {
            ...
            var logInfo = new SysLogDto
            {
                ...
                ExecutionDuration = Convert.ToInt32(stopwatch.Elapsed.TotalMilliseconds)
            };
            if (!string.IsNullOrEmpty(logResult))
            {
                logInfo.ReturnValue = logResult.Replace("\\", "").CutString(1000);
            }
            await _capBus.PublishAsync("log.cap", logInfo);
            //保存日志信息
