...
public class SysRoleService : IApplicationService
{
    ...
    public async Task DeleteAsync([FromBody] List<long> ids)
    {
        ...
        var roleAdmin = _thisRepository.ChangeRepository<SugarRepository<SysAdminRole>>();
        await roleAdmin.DeleteAsync(m => ids.Contains(m.RoleId));
        var permissionRepository = _thisRepository.ChangeRepository<SugarRepository<SysPermission>>();
        await permissionRepository.DeleteAsync(m => ids.Contains(m.RoleId));
    }
