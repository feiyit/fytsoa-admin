...
public class OperatorService : IApplicationService
{
    ...
    public async Task<LoginTokenDto> LoginAsync(LoginParam loginParam)
    {
        ...
        { Id = loginRes.Id, FullName = loginRes.FullName, Role = "Admin", RoleArray = loginRes.RoleGroup.Count > 0 ? string.Join(",", loginRes.RoleGroup) : "0", TenantId = loginRes.TenantId, EmployeeId = loginRes.EmployeeId, Time = DateTime.Now.AddMinutes(60) });
